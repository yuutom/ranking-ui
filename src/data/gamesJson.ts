import { getLatestGameResults } from "./ratingHistoryJson";
...
const GAME_RESULTS_URL = "https://kishi-info.s3.ap-northeast-1.amazonaws.com/game_results.json";

export async function fetchGameResults(): Promise<Game[]> {
  const response = await fetch(GAME_RESULTS_URL);
  if (!response.ok) {
    throw new Error(`Failed to fetch game results: ${response.statusText}`);
  }
  const rawGameResults = await response.json();
  return rawGameResults.flat().map(convertEnums);
}
...
export async function getPickedUpRecentGames(): Promise<any[]> {
  const jsonGameResults = await fetchGameResults();
  ...
  const weeklyResultsWithRating = weeklyResults.map((game) => {
    const senteRating = latestGameResults
      .find((r: any) => r.player_id === game.sente_player_id && r.game_id === game.game_id);
    const goteRating = latestGameResults
      .find((r: any) => r.player_id === game.gote_player_id && r.game_id === game.game_id);
    return {
      ...game,
