...
import { getJsonKishi } from '../data/playersJson'
import { useState, useEffect } from 'react'
import { getFilterdRecord, getRanking, getLatestKishiRatings, getSortedByStreak, getSortedByTotal, getSortedByWinRate, getSortedByWins, getStatsMap } from '../data/ratingHistoryJson'
...
export default function KishiDetail() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const latestKishiRatings = await getLatestKishiRatings();
      setPlayer(jsonKishi.find((k) => String(k.kishiNumber) === kishiNumber));
  ...
  useEffect(() => {
    async function fetchRatings() {
      ...
      const latestKishiRatings = await getLatestKishiRatings();
      const sorted = Array.from(latestKishiRatings.entries())
        .sort((a, b) => b[1].rating - a[1].rating)
        .map(([id]) => id);
      setPlayerRanking(sorted.indexOf(player.id) + 1);
      setPlayerRating(latestKishiRatings.get(player.id)?.rating);
    }
    fetchRatings();
  }, [player]);
  ...
  useEffect(() => {
    async function fetchRankings() {
      if (!player) {
        setWinRateRanking(undefined);
        setTotalRanking(undefined);
        setWinsRanking(undefined);
        setStreakRanking(undefined);
        return;
      }
  ...
  if (loading) {
    return <div className="p-8 text-gray-500">棋士データを読み込み中...</div>;
  }
