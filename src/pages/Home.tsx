...
export default function Home() {
  ...
  if (!jsonKishi || !jsonJoryu) {
    return <div>Loading...</div>;
  }

  return (
    <div className="max-w-screen-md mx-auto mt-4 grid grid-cols-2 gap-5 sm:px-6 lg:grid-flow-col-dense lg:grid-cols-2">
      <section className="lg:col-span-1 lg:col-start-1 h-full flex flex-col">
        <div className="bg-white px-4 py-2 shadow sm:rounded-lg sm:px-6 flex-1 flex flex-col">
          <div className="justify-between items-start">
            ...
            <table className="mt-4 min-w-full divide-y divide-gray-300">
              <thead>
                <tr>
                  <th className="px-1 py-3.5 text-left text-sm">順位</th>
                  <th className="px-3 py-3.5 text-left text-sm">名前</th>
                  <th className="px-3 py-3.5 text-center text-sm">レート</th>
                </tr>
              </thead>
                  ...
                  >
                    <td className="px-3 py-2 text-sm text-gray-800" colSpan={5}>
                      ...
                      <div className="text-sm font-medium text-gray-800 mb-2">
                        <span className="text-xs text-gray-500">{DateUtils.formatShortDate(row.date[0])}  </span>
                        <span className="test-md font-semibold text-gray-600"> {extractDisplayGameName(row.game_name)}</span>
                      </div>
                      ...
                      <div className="grid grid-cols-5 gap-2 text-sm text-gray-800">
                        ...
                        <div className="flex flex-col justify-center items-center col-span-1">
                          ...
                          <div className="text-xs text-gray-500">
                            ({(row.sente_rating - row.sente_rating_delta).toFixed(0)} → {row.sente_rating.toFixed(0)})
                          </div>
                        </div>

                        {/* vs */}
                        <div className="flex justify-center items-center text-gray-500 col-span-1">
                          vs
                        </div>

                        {/* 後手 名前＋レート */}
                        <div className="flex flex-col justify-center items-center col-span-1">
                          <div className="underline">{row.gote_player_name}</div>
                          <div className="text-xs text-gray-500">
                            ({(row.gote_rating - row.gote_rating_delta).toFixed(0)} → {row.gote_rating.toFixed(0)})
                          </div>
                        </div>
